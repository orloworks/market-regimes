...
__export(index_exports, {
  ...
  PERCENTILE_HISTORY_DAYS: () => PERCENTILE_HISTORY_DAYS,
  REGIME_DETECTOR_VERSION: () => REGIME_DETECTOR_VERSION,
  REGIME_TYPES: () => REGIME_TYPES,
  SMA: () => SMA,
  averageStreakLength: () => averageStreakLength,
  clamp: () => clamp,
  classifyRegimeSeries: () => classifyRegimeSeries,
  countCrossovers: () => countCrossovers,
  dailyReturns: () => dailyReturns,
  detectAllRegimes: () => detectAllRegimes,
  detectAllRegimesRaw: () => detectAllRegimesRaw,
  detectChoppy: () => detectChoppy,
  detectCrisis: () => detectCrisis,
  detectInflationary: () => detectInflationary,
  detectNewsDriven: () => detectNewsDriven,
  detectQE: () => detectQE,
  detectTrendDrawdown: () => detectTrendDrawdown,
  detectVolatile: () => detectVolatile,
  excessKurtosis: () => excessKurtosis,
  percentileRank: () => percentileRank,
  rollingKurtosis: () => rollingKurtosis,
  segmentReturnsByRegime: () => segmentReturnsByRegime,
  severityFromPercentile: () => severityFromPercentile,
...
// src/detectors.ts
var import_nanuquant_ts2 = require("nanuquant-ts");

// src/helpers.ts
var import_nanuquant_ts = require("nanuquant-ts");
...
function variance(arr) {
  const m = (0, import_nanuquant_ts.mean)(arr);
  return (0, import_nanuquant_ts.mean)(arr.map((x) => (x - m) ** 2));
}
function excessKurtosis(returns) {
  return (0, import_nanuquant_ts.kurtosis)(returns);
}
function rollingKurtosis(returns, window) {
  ...
  for (let i = 0; i < returns.length; i++) {
    if (i < window - 1) {
      ...
      try {
        result.push((0, import_nanuquant_ts.kurtosis)(slice));
      } catch {
        result.push(NaN);
      }
    }
...
function detectCrisis(data) {
  ...
  if (spyReturns.length >= 21 && tltReturns.length >= 21) {
    ...
    const rollCorr = (0, import_nanuquant_ts2.rollingCorrelation)(spyR, tltR, 21);
...
function detectVolatile(data) {
  ...
  const recent21 = spyReturns.slice(-21);
  const vol21 = (0, import_nanuquant_ts2.std)(recent21, 1) * Math.sqrt(252);
  const volHistory = (0, import_nanuquant_ts2.rollingVolatility)(spyReturns, {
    rollingPeriod: 21,
    periodsPerYear: 252
  });
...
function detectQE(data) {
  ...
  const spyReturn63 = (0, import_nanuquant_ts2.comp)(spyReturns.slice(-63));
  const equityMomentum = clamp(spyReturn63 / 0.1, 0, 1);
  const tltReturn63 = (0, import_nanuquant_ts2.comp)(tltReturns.slice(-63));
  ...
  const vol21 = (0, import_nanuquant_ts2.std)(spyReturns.slice(-21), 1) * Math.sqrt(252);
  const sma50 = SMA(spyPrices, 50);
  const lastSma50 = sma50[sma50.length - 1];
